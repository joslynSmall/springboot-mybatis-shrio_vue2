<template>
  <div class="app-container">
    <div class="filter-container">
      <div class="demo-input-suffix"><span class="name">功能难度等级名称：</span><el-input  v-model="difficultyLevelName" style="width:20%"></el-input>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit highlight-current-row>
      ...
      <el-table-column label="功能难度等级ID">
        <template scope="scope">
          {{scope.row.difficultyLevelId}}
        </template>
      </el-table-column>
      <el-table-column label="功能难度等级名称" align="center">
        <template scope="scope">
          <span>{{scope.row.difficultyLevelName}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="commObj.dialogFormVisible">
      <div slot="title">{{showTitle.title}}-功能难度等级</div>
      <el-form :model="form"label-position="top">
        <el-form-item label="功能难度等级Id" :label-width="formLabelWidth" v-if="showTime">
          <el-input v-model="form.difficultyLevelId" auto-complete="off" :disabled="this.state=='show'"></el-input>
        </el-form-item>
        <el-form-item label="功能难度等级名称" :label-width="formLabelWidth">
          <el-input v-model="form.difficultyLevelName" auto-complete="off" :disabled="this.state=='show'"></el-input>
        </el-form-item>
...
<script>
  import {getDifficultLevelList, getDifficultLevelInfo, insertDifficultLevel, updateDifficultLevel, deleteDifficultLevel} from '@/api/basic'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        this.form.difficultyLevelName = this.difficultyLevelName
        getDifficultLevelList(Object.assign({}, commObj.pages, this.form)).then(response => {
      ...
      insertDifficultLevel() {
        return new Promise((resolve, reject) => {
          insertDifficultLevel(this.form).then(response => {
      ...
      updateDifficultLevel() {
        return new Promise((resolve, reject) => {
          updateDifficultLevel(this.form).then(response => {
            commObj.dialogFormVisible = false
            resolve()
            this.fetchData()
          }).catch(error => {
      ...
      deleteDifficultLevel() {
        return new Promise((resolve, reject) => {
          deleteDifficultLevel(this.params).then(response => {
            commObj.dialogFormVisible = false
            resolve()
            this.fetchData()
          }).catch(error => {
      ...
      getDifficultLevelInfo() {
        return new Promise((resolve, reject) => {
          getDifficultLevelInfo(this.params).then(response => {
            commObj.dialogFormVisible = false
            resolve()
          }).catch(error => {
            reject(error)
      ...
      handleShow(index, row, state)
      {
        ...
        if (state === 'insert')
        {
          ...
          this.form.difficultyLevelName = row.difficultyLevelName;
        ...
        {
          this.form = row;
          this.params.difficultyLevelId = row.difficultyLevelId;
          this.state = 'show';
      ...
      handleSure()
      {
        ...
        {
          this.insertDifficultLevel();
          commObj.dialogFormVisible = true
        }
        else if (this.state === 'update')
        {
          this.updateDifficultLevel();
        }
        else if (this.state === 'show')
        {
          this.getDifficultLevelInfo();
        }
      },
