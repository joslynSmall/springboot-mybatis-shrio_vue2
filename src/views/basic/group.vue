  ...
  <div class="app-container">
    <div class="filter-container">
      <div class="demo-input-suffix">
        <el-input placeholder="请输入项目组名称" v-model="search.groupName" class="input-with-select" style="width:20%"></el-input>
    ...
    <el-table :data="list" v-loading.body="commObj.listLoading" element-loading-text="拼命加载中" border fit highlight-current-row>
      ...
      <el-table-column label="项目组名称" align="center">
        <template scope="scope">
    ...
    <el-dialog :visible.sync="commObj.dialogFormVisible">
      ...
      <el-form :model="form" label-position="top">
        <el-form-item label="项目组名称" :label-width="formLabelWidth">
          <el-input v-model="form.groupName" auto-complete="off" :disabled="this.isEdit=='show'"></el-input>
        </el-form-item>
...
<script>
  import { getGroupList, getGroupInfo, insertGroup, updateGroup, deleteGroup, getProjectList } from '@/api/basic'
    ...
    methods: {
      ...
      getGroupInfo() {
        return new Promise((resolve, reject) => {
          getGroupInfo(this.params).then(response => {
      ...
      insertGroup() {
        return new Promise((resolve, reject) => {
          insertGroup(this.form).then(response => {
            commObj.dialogFormVisible = false
            resolve()
            this.fetchData()
          }).catch(error => {
      ...
      deleteGroup() {
        return new Promise((resolve, reject) => {
          deleteGroup(this.params).then(response => {
            commObj.dialogFormVisible = false
            resolve()
            this.fetchData()
          }).catch(error => {
      ...
      handleShow(index, row, edit) {
        ...
        if (edit === 'insert') {
          ...
          this.form = row
          this.params.groupId = row.groupId
          this.isEdit = 'show'
