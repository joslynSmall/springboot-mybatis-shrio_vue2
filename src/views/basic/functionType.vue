<template>
  <div class="app-container">
    <div class="filter-container">
      <div class="demo-input-suffix"><span class="name">功能类型名称：</span><el-input  v-model="functionTypeName" style="width:20%"></el-input>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit highlight-current-row>
      ...
      <el-table-column label="功能类型ID">
        <template scope="scope">
          {{scope.row.functionTypeId}}
        </template>
      </el-table-column>
      <el-table-column label="功能类型名称" align="center">
        <template scope="scope">
          <span>{{scope.row.functionTypeName}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="commObj.dialogFormVisible">
      <div slot="title">{{showTitle.title}}功能类型</div>
      <el-form :model="form"label-position="top">
        <el-form-item label="功能类型Id" :label-width="formLabelWidth" v-if="showTime">
          <el-input v-model="form.functionTypeId" auto-complete="off" :disabled="this.state=='show'"></el-input>
        </el-form-item>
        <el-form-item label="功能类型名称" :label-width="formLabelWidth">
          <el-input v-model="form.functionTypeName" auto-complete="off" :disabled="this.state=='show'"></el-input>
        </el-form-item>
...
<script>
  import {getFunctionTypeList, getFunctionTypeInfo, insertFunctionType, updateFunctionType, deleteFunctionType} from '@/api/basic'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        ...
        this.form.functionTypeName = this.functionTypeName
        getFunctionTypeList(Object.assign({}, commObj.pages, this.form)).then(response => {
      ...
      insertFunctionType() {
        return new Promise((resolve, reject) => {
          insertFunctionType(this.form).then(response => {
      ...
      updateFunctionType() {
        return new Promise((resolve, reject) => {
          updateFunctionType(this.form).then(response => {
            commObj.dialogFormVisible = false
            resolve()
            this.fetchData()
          }).catch(error => {
      ...
      deleteFunctionType() {
        return new Promise((resolve, reject) => {
          deleteFunctionType(this.params).then(response => {
            commObj.dialogFormVisible = false
            resolve()
            this.fetchData()
          }).catch(error => {
      ...
      getFunctionTypeInfo() {
        return new Promise((resolve, reject) => {
          getFunctionTypeInfo(this.params).then(response => {
            commObj.dialogFormVisible = false
            resolve()
          }).catch(error => {
            reject(error)
      ...
      handleShow(index, row, state)
      {
        ...
        {
          this.form = row;
          this.params.functionTypeId = row.functionTypeId;
          this.state = 'show';
          // this.form.functionTypeName = row.functionTypeName;
      ...
      handleSure()
      {
        ...
        {
          this.insertFunctionType();
          commObj.dialogFormVisible = true
        }
        else if (this.state === 'update')
        {
          this.updateFunctionType();
        }
